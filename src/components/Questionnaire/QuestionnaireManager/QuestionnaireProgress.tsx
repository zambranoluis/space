...
const QuestionnaireProgress: React.FC<QuestionnaireProgressProps> = ({
  ...
  useEffect(() => {
    ...
    if (
      lastAnsweredIndex !== -1 &&
      questionRefsGeneral.current[lastAnsweredIndex]
    ) {
  ...
  useEffect(() => {
    setCountedGeneralAnswers(
      isAnsweredGeneral.filter((isAnswered) => isAnswered).length
    );
  }, [isAnsweredGeneral]);
  ...
  useEffect(() => {
    ...
    if (
      lastAnsweredIndex !== -1 &&
      questionRefsBackyard.current[lastAnsweredIndex]
    ) {
  ...
  useEffect(() => {
    setCountedBackyardAnswers(
      isAnsweredBackyard.filter((isAnswered) => isAnswered).length
    );
  }, [isAnsweredBackyard]);
  ...
  useEffect(() => {
    ...
    if (
      lastAnsweredIndex !== -1 &&
      questionRefsFrontyard.current[lastAnsweredIndex]
    ) {
  ...
  useEffect(() => {
    setCountedFrontyardAnswers(
      isAnsweredFrontyard.filter((isAnswered) => isAnswered).length
    );
  }, [isAnsweredFrontyard]);
  ...
  useEffect(() => {
    ...
    if (
      lastAnsweredIndex !== -1 &&
      questionRefsExtra.current[lastAnsweredIndex]
    ) {
  ...
  useEffect(() => {
    setCountedExtraAnswers(
      isAnsweredExtra.filter((isAnswered) => isAnswered).length
    );
  }, [isAnsweredExtra]);
      ...
      >
        <div className="w-full">
          <div
            ...
            onClick={() => {
              openProgress("openGeneralArrow", "progressGeneral");
            }}
          >
            <MdOutlineExpandLess
              id="openGeneralArrow"
            ...
            <h2 className={`pl4 `}>
              ...
              <span className="text-xs">
                ( {countedGeneralAnswers} / {questionnaire.general.length} )
              </span>
            </h2>
          </div>
          <div
            ...
            <div
              className={`${
                isAnsweredGeneral[isAnsweredGeneral.length - 1]
                  ? "text-white"
                  : "text-gray-200/50"
              } flex justify-center items-center gap-1 bgred-200 place-self-start cursor-pointer`}
              onClick={() => {
                openProgress("openBackyardArrow", "progressBackyard");
              }}
            >
              <MdOutlineExpandLess
                id="openBackyardArrow"
                className="bgred-300 text-lg rotate-180 "
              />
              <h2 className={` `}>
                Backyard Questions:{" "}
                <span className="text-xs">
                  ( {countedBackyardAnswers} / {questionnaire.backyard.length} )
                </span>
              </h2>
            </div>
            <div
              id="progressBackyard"
              className="flex gap-2 w-full select-none overflow-y-hidden p-2 overflow-x-auto transition-all duration-100 questionnaireProgressScroll"
            >
              {questionnaire.backyard.map((answer, index) => (
                <div
                  ...
                  className={`${
                    isAnsweredBackyard[index] ? "bggreen-500" : "bgred-500"
                  } flex gap-2 justify-center items-center px-2 py-1 rounded`}
                >
                  <p
                    className={`${
                      isAnsweredBackyard[index]
                        ? "text-white"
                        : "text-gray-200/50"
                    } text-xs`}
                  >
          ...
          categories.includes("Frontyard") && (
            <div className="w-full">
              <div
                ...
                onClick={() => {
                  openProgress("openFrontyardArrow", "progressFrontyard");
                }}
              >
                <MdOutlineExpandLess
                  id="openFrontyardArrow"
                  className="bgred-300 text-lg rotate-180 "
                />
                <h2 className={` `}>
                  Frontyard Questions:{" "}
                  <span className="text-xs">
                    ( {countedFrontyardAnswers} /{" "}
                    {questionnaire.backyard.length} )
                  </span>
                </h2>
              </div>
              <div
                id="progressFrontyard"
                className="flex gap-2 w-full select-none overflow-y-hidden p-2 overflow-x-auto transition-all duration-100 questionnaireProgressScroll"
              >
                {questionnaire.backyard.map((answer, index) => (
                  <div
                    ...
                    className={`${
                      isAnsweredFrontyard[index] ? "bggreen-500" : "bgred-500"
                    } flex gap-2 justify-center items-center px-2 py-1 rounded`}
                  >
                    <p
                      className={`${
                        isAnsweredFrontyard[index]
                          ? "text-white"
                          : "text-gray-200/50"
                      } text-xs`}
                    >
        ...
        {!categories.includes("Frontyard") && (
          <div className="w-full">
            <div
              ...
              onClick={() => {
                openProgress("openExtraArrow", "progressExtra");
              }}
            >
              <MdOutlineExpandLess
                id="openExtraArrow"
              ...
              <h2 className={` `}>
                Extra Questions:{" "}
                <span className="text-xs">
                  ( {countedExtraAnswers} / {questionnaire.extra.length} )
                </span>
              </h2>
            </div>
            <div
            ...
            >
              {questionnaire.extra.map((answer, index) => (
                <div
                  ...
                  className={`${
                    isAnsweredExtra[index] ? "bggreen-500" : "bgred-500"
                  } flex gap-2 justify-center items-center px-2 py-1 rounded`}
                >
                  <p
                    className={`${
                      isAnsweredExtra[index] ? "text-white" : "text-gray-200/50"
                    } text-xs`}
                  >
                    {index + 1}.
                  </p>
        ...
        <div className="w-full">
          <div
            className="flex justify-center items-center gap-1 bgred-200 place-self-start cursor-pointer"
            onClick={() => {
              openProgress("openMediaArrow", "progressMedia");
            }}
          >
            <MdOutlineExpandLess
              id="openMediaArrow"
