...
interface QuestionnaireExtraProps {
  ...
  setAnswersExtra: React.Dispatch<React.SetStateAction<question[]>>;
  categories: string[];
...
const QuestionnaireExtra: React.FC<QuestionnaireExtraProps> = ({
  ...
  const questionRefsExtra = useRef<Array<HTMLDivElement | null>>(
    new Array(questionnaire.extra.length).fill(null)
  );
  ...
  useEffect(() => {
    if (categories.includes("Frontyard")) {
      if (
        ...
        setContainerHeightExtra(
          (prevHeight) =>
            prevHeight + questionRefsExtra.current![0]!.offsetHeight || 0
        );
      }
    } else {
      if (
  ...
  useEffect(() => {
    const calculateContainerHeightExtra = () => {
      ...
      isAnsweredExtra.forEach((answered, index) => {
        ...
        if (answered && currentElement) {
          ...
          if (
            index + 1 < isAnsweredExtra.length &&
            !isAnsweredExtra[index + 1] &&
            nextElement
          ) {
    ...
    isAnsweredFrontyard,
    isAnsweredExtra,
    questionRefsExtra.current.map((el) => el?.offsetHeight).join(","),
  ]);

  const handleControlText = (value: string, index: number) => {
    const newValue = value;
    setAnswersExtra((prev) =>
    ...
    <section
      ...
      className={`${
        categories.includes("Frontyard")
          ? isAnsweredFrontyard[isAnsweredFrontyard.length - 1] === true
            ? ""
            : "hidden"
          : isAnsweredBackyard[isAnsweredBackyard.length - 1] === true
          ? ""
          : "hidden"
      } flex flex-col w-full justify-center items-center gap-20 `}
    >
      <div
        ref={containerRefExtra}
        className="flex flex-col w-[90%] gap-12 overflow-hidden"
        style={{ height: `${containerHeightExtra}px` }}
      >
        {answersExtra && isAnsweredExtra[0] === true ? (
          <div
            id="eq1"
            ref={(el) => {
              questionRefsExtra.current[0] = el;
            }}
            className={`${
              categories.includes("Frontyard")
                ? isAnsweredFrontyard[isAnsweredFrontyard.length - 1] === true
          ...
          >
            ...
            <div className="flex flex-col gap-6 w-full justify-center items-start p-12 py-20">
              <div className="flex flex-col w-full p-2">
                ...
                <textarea
                  id="eq1Note"
                  value={answersExtra[0].notes[0].note}
                  onChange={(e) => handleControlText(e.target.value, 0)}
        ...
        {answersExtra && isAnsweredExtra[1] === true ? (
          <div
            id="eq2"
            ref={(el) => {
              questionRefsExtra.current[1] = el;
            }}
            className={`${
              isAnsweredExtra[0] === true
                ? ""
                : "-translate-x-[-110%] opacity-0"
            } transition-all duration-1000 flex flex-col bgred-300 rounded-t-[28px] border-2 border-[#68664d] justify-center items-center`}
          >
            ...
            <div className="flex flex-col gap-6 w-full justify-center items-start p-12 py-20">
              <div className="flex flex-col w-full p-2">
                ...
                <textarea
                  id="eq2Note"
                  value={answersExtra[1].notes[0].note}
                  onChange={(e) => handleControlText(e.target.value, 1)}
                  className="bg-[#ebebeb] p-2 text-black outline-none h-[100px]"
                ></textarea>
              </div>
            </div>
        ...
        {answersExtra && isAnsweredExtra[2] === true ? (
          <div
            id="eq3"
            ref={(el) => {
              questionRefsExtra.current[2] = el;
            }}
            className={`${
              isAnsweredExtra[1] === true ? "" : "translate-x-[-110%] opacity-0"
            } transition-all duration-1000 flex flex-col bgred-300 rounded-t-[28px] border-2 border-[#68664d] justify-center items-center`}
          >
            <div className="flex bg-[#68664d] relative pt-4 pl-6 pb-6 text-xl font-black rounded-t-3xl w-full">
              <div className="w-full bggreen-300 p-2 flex">
            ...
            <div className="flex flex-col gap-6 w-full justify-center items-start p-12 py-20">
              <div className="flex flex-col w-full p-2">
                ...
                <textarea
                  id="eq3Note"
                  value={answersExtra[2].notes[0].note}
                  onChange={(e) => {
                    handleControlText(e.target.value, 2);
                  }}
                  className="bg-[#ebebeb] p-2 text-black outline-none h-[100px]"
                ></textarea>
              </div>
        ...
        {answersExtra && isAnsweredExtra[3] === true ? (
          <div
            id="eq4"
            ref={(el) => {
              questionRefsExtra.current[3] = el;
            }}
            className={`${
              isAnsweredExtra[2] === true
                ? ""
                : "-translate-x-[-110%] opacity-0"
            } transition-all duration-1000 flex flex-col bgred-300 rounded-t-[28px] border-2 border-[#68664d] justify-center items-center`}
          >
            ...
            <div className="flex flex-col gap-6 w-full justify-center items-start p-12 py-20">
              <div className="flex flex-col w-full p-2">
                ...
                <textarea
                  id="eq4Note"
                  value={answersExtra[3].notes[0].note}
                  onChange={(e) => {
