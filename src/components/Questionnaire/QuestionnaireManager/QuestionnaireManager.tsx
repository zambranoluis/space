...
const QuestionnaireManager: React.FC = () => {
  ...
  const handleMaxTwoGeneral = (index: number) => {
    if (selectedMaxTwoGeneral.includes(index)) {
      ...
      setSelectedMaxTwoGeneral(selectedMaxTwoGeneral.filter((option) => option !== index));
  ...
  const handleMaxTwoBackyard = (index: number) => {
    if (selectedMaxTwoBackyard.includes(index)) {
      // Si el índice ya está seleccionado, lo eliminamos
      setSelectedMaxTwoBackyard(selectedMaxTwoBackyard.filter((option) => option !== index));
  ...
  const [answersFrontyard, setAnswersFrontyard] = useState<{question: string, answer: string}[]>([]);
  ...
  const handleMaxTwoFrontyard = (index: number) => {
    if (selectedMaxTwoFrontyard.includes(index)) {
      // Si el índice ya está seleccionado, lo eliminamos
      setSelectedMaxTwoFrontyard(selectedMaxTwoFrontyard.filter((option) => option !== index));
  ...
  const [answersExtra, setAnswersExtra] = useState<{question: string, answer: string}[]>([]);
